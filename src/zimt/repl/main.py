...
def _help() -> None:
    ...
    print("prompt syntax (two-pass; deterministic per seed):")
    print("  [a|b|c]              alternation (only when `|` is present;")
    print("                        bare [word] stays literal for compel)")
    print("  [2::a|1::b]          weighted alternation (2/3 a, 1/3 b)")
    print("  [a [b|c]|d]          nesting; [|a] means 'maybe a, maybe nothing'")
    print("  ${c=value}           bind variable (silent); pick once per seed")
    print("  ${c}                 reference the bound value (reuse the pick)")
    print("  ${o={k=v, k=v}}      JSON-like composite: binds o.k for each field")
    print("  ${o.k}               access a composite field (nested OK: o.a.b)")
    print("  ${c=`raw text`}      verbatim: stored literally, re-parsed per ref")
    print("                        (each ${c} rolls a new alternation pick)")
    print("  \\[ \\| \\$ \\`           escape for literal brackets / pipe / $ / `")
